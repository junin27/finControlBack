...
public class BankService {
    ...
    public BankDto update(UUID id, BankUpdateDto dto) {
        ...
        Bank b = bankRepo.findByIdAndUserId(id, currentUser.getId())
                .orElseThrow(() -> new ResourceNotFoundException("Banco não encontrado com ID: " + id + " para o usuário atual."));
        ...
        if (needsUpdate) {
            bankRepo.save(b);
            log.info("Banco {} (ID: {}) atualizado para o usuário {}", b.getName(), b.getId(), currentUser.getId());
        }
        return toDto(b);
    }
    ...
    public void deleteAll(UUID id) {
        User currentUser = getCurrentUser();
        Bank b = bankRepo.findByIdAndUserId(id, currentUser.getId())
                .orElseThrow(() -> new ResourceNotFoundException("Banco não encontrado com ID: " + id + " para o usuário atual ao tentar deletar."));
        bankRepo.deleteById(b.getId());
        log.info("Banco {} (ID: {}) deletado para o usuário {}", b.getName(), b.getId(), currentUser.getId());
    }
    ...
    public void clearExpenses(UUID bankId) {
        ...
        BigDecimal totalExpensesCleared = expenseRepo.sumExpenseByBank(bankId);
        if (totalExpensesCleared == null) totalExpensesCleared = BigDecimal.ZERO;

        expenseRepo.deleteByBankId(bankId); // Certifique-se que este método existe no expenseRepo

        bank.setBalance(bank.getBalance().add(totalExpensesCleared));
        bankRepo.save(bank);
        log.info("Despesas do banco {} (ID: {}) limpas e saldo atualizado para o usuário {}", bank.getName(), bankId, currentUser.getId());
    }
    ...
    public BankTransferResponseDto transferBetweenBanks(BankTransferDto transferDto) {
        ...
        Bank sourceBank = bankRepo.findByIdAndUserId(transferDto.getSourceBankId(), currentUserId)
                .orElseThrow(() -> new ResourceNotFoundException("Banco de origem não encontrado com ID: " + transferDto.getSourceBankId() + " ou não pertence ao usuário."));

        Bank destinationBank = bankRepo.findByIdAndUserId(transferDto.getDestinationBankId(), currentUserId)
                .orElseThrow(() -> new ResourceNotFoundException("Banco de destino não encontrado com ID: " + transferDto.getDestinationBankId() + " ou não pertence ao usuário."));
        ...
        if (sourceBalanceBefore.compareTo(transferDto.getAmount()) < 0) {
            throw new InsufficientBalanceException(ERROR_TRANSFER_INSUFFICIENT_FUNDS +
    ...
    public List<BankDto> updateAllBanks(List<BankBulkUpdateItemDto> dtoList) {
        ...
        for (BankBulkUpdateItemDto dto : dtoList) {
            Bank b = bankRepo.findByIdAndUserId(dto.getId(), currentUser.getId())
                    .orElseThrow(() -> new ResourceNotFoundException("Banco com ID " + dto.getId() + " não encontrado ou não pertence ao usuário."));
            ...
            if (needsUpdate) {
                ...
                updatedBanksList.add(b);
            }
        }
        log.info("Atualização em massa de {} bancos processada para o usuário {}", dtoList.size(), currentUser.getId());
        return updatedBanksList.stream().map(this::toDto).collect(Collectors.toList());
    }
    ...
    public void deleteAllUserBanks() {
        ...
        if (!userBanks.isEmpty()) {
            // Se você implementou deleteAllByUserId no BankRepository, use-o para melhor performance:
            // bankRepo.deleteAllByUserId(currentUser.getId());
            // log.info("Todos os bancos do usuário {} foram deletados via deleteAllByUserId.", currentUser.getId());
            bankRepo.deleteAll(userBanks); // Alternativa
            log.info("Todos os {} bancos do usuário {} foram deletados.", userBanks.size(), currentUser.getId());
        } else {
    ...
    public BankDto addMoneyToBank(UUID bankId, AmountDto dto) {
        ...
        bankRepo.save(b);
        log.info("Adicionado {} ao saldo do banco {} (ID: {}) para o usuário {}", dto.getAmount(), b.getName(), bankId, currentUser.getId());
        return toDto(b);
    }
    ...
    public List<BankDto> addMoneyToAllBanks(AmountDto dto) {
        ...
        log.info("Adicionado {} ao saldo de {} bancos para o usuário {}", dto.getAmount(), savedBanks.size(), currentUser.getId());
        return savedBanks.stream().map(this::toDto).collect(Collectors.toList());
    }
    ...
    public BankDto removeMoneyFromBank(UUID bankId, AmountDto dto) {
        ...
        if (b.getBalance().compareTo(dto.getAmount()) < 0) {
            throw new InsufficientBalanceException(ERROR_INSUFFICIENT_FUNDS_GENERIC + " Saldo atual no banco '" + b.getName() + "': " + b.getBalance() + ". Tentativa de remover: " + dto.getAmount());
        }

        b.setBalance(b.getBalance().subtract(dto.getAmount()));
        bankRepo.save(b);
        log.info("Removido {} do saldo do banco {} (ID: {}) para o usuário {}", dto.getAmount(), b.getName(), bankId, currentUser.getId());
        return toDto(b);
    }
    ...
    public List<BankDto> removeMoneyFromAllBanks(AmountDto dto) {
        ...
        for (Bank b : userBanks) {
            if (b.getBalance().compareTo(dto.getAmount()) < 0) {
                throw new InsufficientBalanceException("Falha ao remover dinheiro de todos os bancos: O banco '" + b.getName() + "' (ID: " + b.getId() + ") possui saldo insuficiente (" + b.getBalance() + ") para a remoção de " + dto.getAmount() + ".");
            }
            b.setBalance(b.getBalance().subtract(dto.getAmount()));
        }

        List<Bank> savedBanks = bankRepo.saveAll(userBanks);
        log.info("Removido {} do saldo de {} bancos para o usuário {}", dto.getAmount(), savedBanks.size(), currentUser.getId());
        return savedBanks.stream().map(this::toDto).collect(Collectors.toList());
    }
    ...
    public BankMetricsDto getBankMetrics() {
        ...
        if (userBanks.isEmpty()) {
            ...
            return new BankMetricsDto(BigDecimal.ZERO, null, null, BigDecimal.ZERO, null, null, 0L, 0L, null, null, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO);
        }
        ...
        BigDecimal averageBalancePerBank = totalBalanceAllBanks.divide(BigDecimal.valueOf(userBanks.size()), 2, RoundingMode.HALF_UP);

        List<ExtraIncome> allIncomes = new ArrayList<>();
        List<Expense> allExpenses = new ArrayList<>();

        for (Bank bank : userBanks) {
            allIncomes.addAll(bank.getIncomes() != null ? bank.getIncomes() : Collections.emptyList());
            allExpenses.addAll(bank.getExpenses() != null ? bank.getExpenses() : Collections.emptyList());
        }
        ...
        Expense highestExpenseEntity = allExpenses.stream()
                .filter(e -> e.getValue() != null) // Adicionado para evitar NPE se getValue() puder ser nulo
        ...
        if (highestExpenseEntity != null && highestExpenseEntity.getBank() != null) {
            highestExpenseDto = new BankTransactionDetailsDto(highestExpenseEntity.getBank().getId(), highestExpenseEntity.getBank().getName(), highestExpenseEntity.getValue(), highestExpenseEntity.getDescription());
        }
        ...
        if (!userBanks.isEmpty()) {
            ...
            if (bankWithMostIncomes != null) {
                mostIncomesDto = new BankActivityCountDetailsDto(bankWithMostIncomes.getId(), bankWithMostIncomes.getName(), bankWithMostIncomes.getIncomes() != null ? bankWithMostIncomes.getIncomes().size() : 0);
            }
        }
        ...
        if (!userBanks.isEmpty()){
            ...
            if (bankWithMostExpenses != null) {
                mostExpensesDto = new BankActivityCountDetailsDto(bankWithMostExpenses.getId(), bankWithMostExpenses.getName(), bankWithMostExpenses.getExpenses() != null ? bankWithMostExpenses.getExpenses().size() : 0);
            }
        }

        BigDecimal totalIncomeValue = allIncomes.stream()
                .map(ExtraIncome::getAmount)
                .filter(Objects::nonNull) // Adicionado para evitar NPE
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        BigDecimal avgIncomeValue = userBanks.isEmpty() ? BigDecimal.ZERO : totalIncomeValue.divide(BigDecimal.valueOf(userBanks.size()), 2, RoundingMode.HALF_UP);
        BigDecimal avgExpenseValue = userBanks.isEmpty() ? BigDecimal.ZERO : totalExpenseValue.divide(BigDecimal.valueOf(userBanks.size()), 2, RoundingMode.HALF_UP);
        BigDecimal avgIncomeCount = userBanks.isEmpty() ? BigDecimal.ZERO : BigDecimal.valueOf(totalExtraIncomesCount).divide(BigDecimal.valueOf(userBanks.size()), 2, RoundingMode.HALF_UP);
        BigDecimal avgExpenseCount = userBanks.isEmpty() ? BigDecimal.ZERO : BigDecimal.valueOf(totalExpensesCount).divide(BigDecimal.valueOf(userBanks.size()), 2, RoundingMode.HALF_UP);

        log.debug("Métricas calculadas para o usuário {}", currentUser.getId());
        return new BankMetricsDto(
                totalBalanceAllBanks,
                bankWithHighestBalance != null ? new BankBalanceDetailsDto(bankWithHighestBalance.getId(), bankWithHighestBalance.getName(), bankWithHighestBalance.getBalance()) : null,
                bankWithLowestBalance != null ? new BankBalanceDetailsDto(bankWithLowestBalance.getId(), bankWithLowestBalance.getName(), bankWithLowestBalance.getBalance()) : null,
                averageBalancePerBank,
                highestExpenseDto,
                highestIncomeDto,
